...
1. **Snapshot before touching history**
   - Check `git status`.
   - Create a unique local reference to the original source, such as `git branch backup/original-large-pr` for a manual split or a timestamped `backup/deltathread-*` reference for an automated build. Never move or overwrite that reference during the split.
...
6. **Verify each PR independently**
   ...
   - Every changed test file MUST be in the same PR as the changed production implementation it directly validates. This rule is stricter than the preferred line budget: keep the pair together even when it requires a documented 501-1000 line exception.
   - A later "tests-only" PR is not allowed for tests introduced or updated by this change. Tests may be separate only when they validate unchanged pre-existing production behavior, and the plan must state that fact explicitly.
   - Do not leave required docs or generated files separated from the code they describe.
